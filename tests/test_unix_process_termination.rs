...
/// Check if a process is still running (using try_wait to avoid zombie false positives)
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    // If it returns Ok(None), the process is still running.
    // If it returns Ok(Some(status)), it has terminated.
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, robust loop
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Give it a moment to register the trap before sending SIGTERM
