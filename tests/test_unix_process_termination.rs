...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, sleep in loop to avoid exec optimization
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Wait a short time to allow the shell to register the trap handler
    ...
    // Wait for the process to be reaped or kill it.
    // Ignore ESRCH since it might have already exited depending on timing.
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Use bash instead of claude so it runs the script without needing claude installed
