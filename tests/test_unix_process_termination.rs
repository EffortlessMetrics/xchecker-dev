...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, wait in a loop
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Give the signal trap time to register
...
async fn test_graceful_termination_with_sigterm() -> Result<()> {
    ...
    // Give the process time to spawn and setup
    sleep(Duration::from_millis(1000)).await;
...
async fn test_timeout_grace_period() -> Result<()> {
    ...
    // Give process time to spawn and setup
    sleep(Duration::from_millis(1000)).await;
