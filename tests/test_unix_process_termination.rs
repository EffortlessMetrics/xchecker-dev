...
/// Check if a process is running by trying to wait on the child without blocking.
/// This reliably avoids false positives for zombie processes.
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    ...
    // If it returns Ok(Some(status)), it has exited.
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait a moment for process to initialize handlers
