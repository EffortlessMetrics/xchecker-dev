...
fn is_process_running(child: &mut tokio::process::Child) -> bool {
    // If try_wait() returns None, the process is still running.
    // If it returns Some(status), the process has terminated.
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, loop indefinitely
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Give time for trap to register
    sleep(Duration::from_millis(1000)).await;

    // Send SIGTERM (process will ignore it)
    // Ignore ESRCH on the second signal, as the process may have already exited due to the first signal or timing conditions
