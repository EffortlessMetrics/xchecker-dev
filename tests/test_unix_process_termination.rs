...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait for the process to actually start and register signal handlers
    ...
    if is_process_running(pid) {
        println!("Warning: Process is still running after SIGKILL");
    }
...
async fn test_graceful_termination_with_sigterm() -> Result<()> {
    ...
    // Wait for the process to start and register signal handlers
    sleep(Duration::from_millis(2000)).await;
    ...
    // Wait for graceful termination (poll multiple times)
    for _ in 0..10 {
        if !is_process_running(pid) {
            break;
        }
    ...
    if is_process_running(pid) {
        println!("Warning: Process is still running after SIGTERM");
    }
...
async fn test_process_group_termination() -> Result<()> {
    ...
    if is_process_running(parent_pid) {
        println!("Warning: Parent process is still running after SIGKILL");
    }
