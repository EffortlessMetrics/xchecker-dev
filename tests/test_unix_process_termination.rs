...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // We use a loop to prevent shell 'exec' optimization and ensure the shell
    // process remains alive even if the sleep command is killed or terminates.
    ...
    // Give process time to register signal handler
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Override claude_path to use bash directly, as 'claude' is not available in CI
