...
/// Robustly check if a process is running using try_wait() to avoid zombie false positives
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Spawn a process that robustly ignores SIGTERM (to test SIGKILL)
    ...
    // Wait for process to theoretically process the signal
    sleep(Duration::from_millis(1000)).await;
...
async fn test_graceful_termination_with_sigterm() -> Result<()> {
    ...
    // Wait for signal handler registration setup
    sleep(Duration::from_millis(1000)).await;
...
async fn test_timeout_grace_period() -> Result<()> {
    ...
    // Wait for child processes to start properly
    sleep(Duration::from_millis(1000)).await;
