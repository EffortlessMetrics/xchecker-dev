...
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    ...
    // We avoid kill(pid, 0) because it can return true for zombie processes
    // in some CI environments, leading to false positives.
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait a short time for signal handler registration
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Mock claude executable to avoid "No such file or directory" error
