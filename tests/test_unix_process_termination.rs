...
/// Check if a process is still running via its child handle to avoid zombie false positives
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Robustly ignore SIGTERM without exec optimizations
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Wait for signal handler registration
    ...
    // Wait a sufficient time for termination
    sleep(Duration::from_millis(1000)).await;
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Mock the claude executable to avoid 'No such file or directory' errors in CI
