...
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    // If try_wait returns Ok(None), the child is still running.
    // If it returns Ok(Some(status)), the child has terminated.
    // If it returns an error, assume it's no longer running.
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait briefly to ensure the shell process has initialized and registered its trap handler.
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Mock the claude path to avoid "No such file or directory" since claude isn't installed in the CI test env.
