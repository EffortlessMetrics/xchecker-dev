...
/// We cannot rely on `kill(pid, 0)` as it returns true for zombie processes.
/// We use `child.try_wait()?.is_none()` where possible, but for process groups
/// we may need this fallback, though using try_wait on the child is preferred.
fn is_process_running(pid: u32) -> bool {
    ...
    // Note: This is unreliable for zombie processes. See tests below for correct usage.
    kill(pid, None).is_ok()
}
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, loop to prevent shell exec optimization
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Wait for signal handler to register
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // We mock the binary by prepending the temporary directory to the PATH environment variable
    // because `Runner::native` expects the `claude` binary to be available. We create a script
    // named `claude` that delegates to our script.
    let claude_path = temp_dir.path().join("claude");
    create_test_script(claude_path.to_str().unwrap(), 60)?;

    // Modify runner to execute our script directly by using `wsl_options.claude_path`
    let mut runner = Runner::native();
    runner.wsl_options.claude_path = Some(claude_path.to_str().unwrap().to_string());

    let timeout_duration = Some(Duration::from_secs(1));

    let result = runner.execute_claude(&[], "", timeout_duration).await;
