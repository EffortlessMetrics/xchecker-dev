...
async fn test_process_group_creation() -> Result<()> {
    ...
    let mut child = cmd.spawn()?; sleep(Duration::from_millis(1500)).await; sleep(Duration::from_millis(1500)).await; sleep(Duration::from_millis(1500)).await; sleep(Duration::from_millis(50)).await; sleep(Duration::from_millis(50)).await; sleep(Duration::from_millis(5000)).await;
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    let mut runner = Runner::native(); runner.wsl_options.claude_path = Some("bash".to_string());
