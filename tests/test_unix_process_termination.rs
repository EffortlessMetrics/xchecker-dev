...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Check via try_wait() to handle zombies correctly (is_process_running returns true for zombies)
    match child.try_wait() {
        Ok(Some(_)) => {} // Process exited
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Use bash instead of claude since claude might not be installed
