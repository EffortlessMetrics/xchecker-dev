...
/// Helper function to check process status via the child's wait handle
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait 1 second to ensure signal handler is registered
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Create a runner with a short timeout and mock the claude command
