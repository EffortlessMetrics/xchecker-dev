...
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    match child.try_wait() {
        ...
        Err(_) => false,      // Error getting status (assume not running)
    }
}
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("exec sh -c 'trap \"\" TERM; while true; do sleep 1; done'") // Robust SIGTERM ignore loop preventing shell exit
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Allow shell initialization to complete before checking state or sending signals
    ...
    // Wait a short time (increased to 1000ms for CI robustness)
    sleep(Duration::from_millis(1000)).await;
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Use bash as a dummy for claude to avoid "No such file or directory" error
