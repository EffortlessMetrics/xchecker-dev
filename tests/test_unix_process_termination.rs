...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    match child.try_wait()? {
        Some(_) => {} // Process exited as expected
        None => panic!("Process should be terminated after SIGKILL"),
    }
...
async fn test_graceful_termination_with_sigterm() -> Result<()> {
    ...
    match child.try_wait()? {
        Some(_) => {} // Process exited as expected
        None => panic!("Process should be terminated after SIGTERM"),
    }
...
async fn test_process_group_termination() -> Result<()> {
    ...
    match child.try_wait()? {
        Some(_) => {} // Process exited as expected
        None => panic!("Parent process should be terminated"),
    }
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Configure to use bash instead of claude since claude is not in CI
