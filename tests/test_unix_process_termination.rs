...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait for bash and signal handlers to initialize
    ...
    // Just in case wait one more second if it isn't terminated
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Override the mock claude executable path to be bash so it can execute our script without Os Error 2
