...
/// Check if a process is still running via child.try_wait()
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait a short time to allow the trap to register
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // mock the claude path to avoid "No such file or directory" error in CI
