...
/// Check if a process is still running using child.try_wait() to avoid zombie false positives
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, loop to prevent exec optimization
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // Wait for the script to start and register trap
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // In CI environments where `claude` binary might not exist, mock it to `bash`
