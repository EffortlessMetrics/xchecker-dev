...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, prevent shell exec
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    assert!(
        child.try_wait()?.is_some() || !is_process_running(pid),
...
async fn test_process_group_termination() -> Result<()> {
    ...
    assert!(
        child.try_wait()?.is_some() || !is_process_running(parent_pid),
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    runner.wsl_options.claude_path = Some("sh".to_string()); // Use standard binary for test without claude
