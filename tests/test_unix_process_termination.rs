...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    let mut cmd = CommandSpec::new("sh")
        .arg("-c")
        .arg("trap '' TERM; while true; do sleep 1; done") // Ignore SIGTERM, sleep robustly
        .to_tokio_command();
    cmd.stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
async fn test_process_group_termination() -> Result<()> {
    ...
    // Wait a bit for child processes to spawn and register signal handlers
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    runner.wsl_options.claude_path = Some("bash".to_string()); // mock claude
