...
/// This avoids false positives with zombie processes by properly reaping them
fn is_process_running_via_child(child: &mut tokio::process::Child) -> bool {
    // try_wait() returns:
    // Ok(None) -> still running
    // Ok(Some(status)) -> exited, and we reaped it
    // Err(e) -> error attempting to wait
...
async fn test_sigterm_then_sigkill_sequence() -> Result<()> {
    ...
    // Wait for the trap to be registered in the shell
...
async fn test_runner_timeout_terminates_process_group() -> Result<()> {
    ...
    // Mock the claude path to use bash instead so it doesn't fail with ENOENT
