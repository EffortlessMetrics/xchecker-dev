...
static DESIGN_COMP_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+(Component[:\s]|[A-Z]\w+\s+Component)").unwrap());
static DESIGN_INTF_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+(Interface[:\s]|[A-Z]\w+\s+(API|Interface))").unwrap());
...
static TASK_SUBTASK_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?m)^\s*[-*]\s+\[[ xX]\]").unwrap());
static TASK_MILESTONE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Milestone").unwrap());
static TASK_DEP_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?im)(Depends\s+on:|Dependencies:)").unwrap());
...
pub fn summarize_requirements(markdown: &str) -> RequirementsSummary {
    ...
    let mut summary = RequirementsSummary {
        user_story_count: REQ_USER_STORY_RE.find_iter(markdown).count(),
        acceptance_criteria_count: REQ_EARS_RE.find_iter(markdown).count(),
        nfr_count: REQ_NFR_RE.find_iter(markdown).count(),
...
pub fn summarize_design(markdown: &str) -> DesignSummary {
    ...
    DesignSummary {
        ...
        component_count: DESIGN_COMP_RE.find_iter(markdown).count(),
        interface_count: DESIGN_INTF_RE.find_iter(markdown).count(),
...
pub fn summarize_tasks(markdown: &str) -> TasksSummary {
    ...
    TasksSummary {
        ...
        subtask_count: TASK_SUBTASK_RE.find_iter(markdown).count(),
        milestone_count: TASK_MILESTONE_RE.find_iter(markdown).count(),
        dependency_count: TASK_DEP_RE.find_iter(markdown).count(),
        ..Default::default()
    }
}
