...
pub fn summarize_requirements(markdown: &str) -> RequirementsSummary {
    ...
    static USER_STORY_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*\*\*User\s+Story[:\*]").unwrap());
    ...
    static EARS_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)(WHEN\s+.+\s+THEN\s+.+\s+SHALL|GIVEN\s+.+\s+WHEN\s+.+\s+THEN)").unwrap());
    summary.acceptance_criteria_count = EARS_RE.find_iter(markdown).count();

    // Match NFR patterns: **NFR-*, NFR-*, **Non-Functional*
    static NFR_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)(^\s*\*\*NFR[-\s]|\bNFR-\w+\b|^\s*\*\*Non-Functional)").unwrap());
    ...
    static REQ_HEADING_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Requirement\s+\d+").unwrap());
...
pub fn summarize_design(markdown: &str) -> DesignSummary {
    ...
    static ARCH_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{1,3}\s+(Architecture|System\s+Architecture)").unwrap());
    ...
    static COMPONENT_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+(Component[:\s]|[A-Z]\w+\s+Component)").unwrap());
    summary.component_count = COMPONENT_RE.find_iter(markdown).count();

    // Count interfaces: ### Interface: X or ## Interface: X or ### X API
    static INTERFACE_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+(Interface[:\s]|[A-Z]\w+\s+(API|Interface))").unwrap());
    summary.interface_count = INTERFACE_RE.find_iter(markdown).count();

    // Count data models: ## Data Model or ### Model: or ### Schema:
    static MODEL_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+(Data\s+Model|Model[:\s]|Schema[:\s]|Entity[:\s])").unwrap());
...
pub fn summarize_tasks(markdown: &str) -> TasksSummary {
    ...
    static TASK_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Task\s+\d+").unwrap());
    summary.task_count = TASK_RE.find_iter(markdown).count();

    // Count subtasks: checkbox items - [ ] or - [x]
    static SUBTASK_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?m)^\s*[-*]\s+\[[ xX]\]").unwrap());
    summary.subtask_count = SUBTASK_RE.find_iter(markdown).count();

    // Count milestones: ## Milestone or ### Milestone
    static MILESTONE_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Milestone").unwrap());
    summary.milestone_count = MILESTONE_RE.find_iter(markdown).count();

    // Count dependencies: "Depends on:" or "Dependencies:"
    static DEP_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r"(?im)(Depends\s+on:|Dependencies:)").unwrap());
