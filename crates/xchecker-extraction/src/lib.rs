...
pub fn summarize_requirements(markdown: &str) -> RequirementsSummary {
    ...
    // Statically cache Regex compilation using LazyLock to avoid per-call compilation overhead.
    // Match user story patterns: **User Story:** or **User Story**:
    static USER_STORY_RE: std::sync::LazyLock<Regex> =
        std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*\*\*User\s+Story[:\*]").unwrap());
    ...
    static NFR_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    ...
    static REQ_HEADING_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        Regex::new(r"(?im)^\s*#{2,3}\s+Requirement\s+\d+").unwrap()
    });
...
pub fn summarize_design(markdown: &str) -> DesignSummary {
    ...
    static ARCH_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
...
pub fn summarize_tasks(markdown: &str) -> TasksSummary {
    ...
    static TASK_RE: std::sync::LazyLock<Regex> =
        std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Task\s+\d+").unwrap());
    summary.task_count = TASK_RE.find_iter(markdown).count();

    // Count subtasks: checkbox items - [ ] or - [x]
    static SUBTASK_RE: std::sync::LazyLock<Regex> =
    ...
    static MILESTONE_RE: std::sync::LazyLock<Regex> =
        std::sync::LazyLock::new(|| Regex::new(r"(?im)^\s*#{2,3}\s+Milestone").unwrap());
    summary.milestone_count = MILESTONE_RE.find_iter(markdown).count();

    // Count dependencies: "Depends on:" or "Dependencies:"
    static DEP_RE: std::sync::LazyLock<Regex> =
        std::sync::LazyLock::new(|| Regex::new(r"(?im)(Depends\s+on:|Dependencies:)").unwrap());
