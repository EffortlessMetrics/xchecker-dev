...
pub fn summarize_tasks(markdown: &str) -> TasksSummary {
    ...
    static TASK_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        Regex::new(r"(?im)^\s*#{2,3}\s+Task\s+\d+").unwrap()
    });
    summary.task_count = TASK_RE.find_iter(markdown).count();

    // Count subtasks: checkbox items - [ ] or - [x]
    static SUBTASK_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        Regex::new(r"(?m)^\s*[-*]\s+\[[ xX]\]").unwrap()
    });
    summary.subtask_count = SUBTASK_RE.find_iter(markdown).count();

    // Count milestones: ## Milestone or ### Milestone
    static MILESTONE_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        Regex::new(r"(?im)^\s*#{2,3}\s+Milestone").unwrap()
    });
    summary.milestone_count = MILESTONE_RE.find_iter(markdown).count();

    // Count dependencies: "Depends on:" or "Dependencies:"
    static DEP_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        Regex::new(r"(?im)(Depends\s+on:|Dependencies:)").unwrap()
    });
