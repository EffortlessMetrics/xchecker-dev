...
pub fn summarize_requirements(markdown: &str) -> RequirementsSummary {
    // ⚡ Bolt: Using statically cached regexes via LazyLock avoids expensive regex compilation on every call,
    // reducing CPU usage and allocations significantly during requirement extraction.
...
pub fn summarize_design(markdown: &str) -> DesignSummary {
    // ⚡ Bolt: Using statically cached regexes via LazyLock avoids expensive regex compilation on every call,
    // reducing CPU usage and allocations significantly during design extraction.
...
pub fn summarize_tasks(markdown: &str) -> TasksSummary {
    // ⚡ Bolt: Using statically cached regexes via LazyLock avoids expensive regex compilation on every call,
    // reducing CPU usage and allocations significantly during task extraction.
