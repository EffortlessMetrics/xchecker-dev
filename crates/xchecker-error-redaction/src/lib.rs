...
pub fn redact_error_message_for_logging(message: &str) -> String {
    ...
    let mut redacted = API_KEY_REGEX.replace_all(message, "[REDACTED_KEY]");
    ...
    if !replacements.is_empty() {
        ...
        redacted = std::borrow::Cow::Owned(string_redacted);
    }
    ...
    if URL_WITH_CREDS_REGEX.is_match(&redacted) {
    ...
    if redacted.contains("password") || redacted.contains("token") {
        ...
        #[allow(clippy::collapsible_if)]
        if PASSWORD_REGEX.is_match(&redacted) {
...
pub fn redact_paths(message: &str) -> String {
    ...
    let mut redacted = UNIX_HOME_REGEX.replace_all(message, "[HOME]");
    ...
    #[allow(clippy::collapsible_if)]
    if WIN_HOME_REGEX.is_match(&redacted) {
    ...
    #[allow(clippy::collapsible_if)]
    if DRIVE_REGEX.is_match(&redacted) {
