...
pub fn redact_error_message_for_logging(message: &str) -> String {
    ...
    static API_KEY_REGEX: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(?:sk-|pk_|api_key|secret|Bearer )[a-zA-Z0-9_-]{20,}").unwrap());
    ...
    static URL_WITH_CREDS_REGEX: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"https?://[a-zA-Z0-9_]+:[^:@\s]+@").unwrap());
...
pub fn redact_paths(message: &str) -> String {
    ...
    static WIN_HOME_REGEX: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(?i)(?:[A-Za-z]:)?\\\\Users\\\\[^\\\\/]+").unwrap());
