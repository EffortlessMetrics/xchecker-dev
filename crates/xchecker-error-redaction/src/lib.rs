...
static API_KEY_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:sk-|pk_|api_key|secret|Bearer )[a-zA-Z0-9_-]{20,}").unwrap());
static LONG_KEY_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[a-zA-Z0-9_-]{32,}").unwrap());
static URL_WITH_CREDS_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"https?://[a-zA-Z0-9_]+:[^:@\s]+@").unwrap());
static PASSWORD_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)(password|pass|token)").unwrap());

static UNIX_HOME_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"/(?:home|Users)/[^/\\\\]+").unwrap());
static WIN_HOME_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)(?:[A-Za-z]:)?\\\\Users\\\\[^\\\\/]+").unwrap());
static DRIVE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[A-Za-z]:\\\\").unwrap());
