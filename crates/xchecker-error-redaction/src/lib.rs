...
pub fn redact_error_message_for_logging(message: &str) -> String {
    ...
    static API_KEY_REGEX: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| regex::Regex::new(r"(?:sk-|pk_|api_key|secret|Bearer )[a-zA-Z0-9_-]{20,}").unwrap());
    ...
    static LONG_KEY_REGEX: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| regex::Regex::new(r"[a-zA-Z0-9_-]{32,}").unwrap());
    ...
    static URL_WITH_CREDS_REGEX: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| regex::Regex::new(r"https?://[a-zA-Z0-9_]+:[^:@\s]+@").unwrap());
    ...
    if redacted.contains("password") || redacted.contains("token") {
        ...
        static PASSWORD_REGEX: std::sync::LazyLock<regex::Regex> =
            std::sync::LazyLock::new(|| regex::Regex::new(r"(?i)(password|pass|token)").unwrap());
...
pub fn redact_paths(message: &str) -> String {
    ...
    static UNIX_HOME_REGEX: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| regex::Regex::new(r"/(?:home|Users)/[^/\\\\]+").unwrap());
    ...
    static WIN_HOME_REGEX: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| regex::Regex::new(r"(?i)(?:[A-Za-z]:)?\\\\Users\\\\[^\\\\/]+").unwrap());
    redacted = WIN_HOME_REGEX.replace_all(&redacted, "[HOME]").to_string();

    // Redact Windows drive letters (C:\, D:\, etc.)
    static DRIVE_REGEX: std::sync::LazyLock<regex::Regex> =
