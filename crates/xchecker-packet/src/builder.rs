...
fn process_candidate_file(
    ...
    // Optimization: Call scan_for_secrets once to avoid double scanning
    ...
    let file_content = if let Some(cache_mutex) = cache {
        ...
        if let Some(insights) = cached_insights {
            ...
            // Cache miss
            // Optimization: We know there are no secrets because we checked above.
            // So we can skip redact_content which would re-scan.
