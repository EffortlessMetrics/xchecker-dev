...
fn process_candidate_file(
    ...
    // Security: Open file first, then check metadata to prevent TOCTOU vulnerabilities
    ...
    std::io::Read::take(&mut file, max_file_size + 1)
    ...
    if content.len() as u64 > max_file_size {
        if candidate.priority == Priority::Upstream {
            return Err(anyhow::anyhow!(
                "Upstream file {} exceeds size limit of {} bytes after reading.",
                candidate.path,
                max_file_size
            ));
        }
        ...
        tracing::warn!(
            "Skipping file that grew during read: {} (> limit {})",
            candidate.path,
            max_file_size
        );
        return Ok(None);
