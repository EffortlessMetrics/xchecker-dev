...
impl FixupParser {
    ...
    pub fn detect_fixup_markers(&self, content: &str) -> Option<String> {
        ...
        static FIXUP_PLAN_REGEX: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r"(?i)FIXUP PLAN:").unwrap());
        static NEEDS_FIXUPS_REGEX: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r"(?i)needs fixups").unwrap());
    ...
    fn extract_diff_blocks(&self, content: &str) -> Result<Vec<UnifiedDiff>, FixupError> {
        ...
        static DIFF_BLOCK_REGEX: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r"(?s)```diff\n(.*?)\n```").unwrap());
    ...
    fn parse_hunks(&self, lines: &[&str], block_index: usize) -> Result<Vec<DiffHunk>, FixupError> {
        ...
        static HUNK_HEADER_REGEX: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
