...
static FIXUP_PLAN_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)FIXUP PLAN:").unwrap());
static NEEDS_FIXUPS_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)needs fixups").unwrap());
static DIFF_BLOCK_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?s)```diff\n(.*?)\n```").unwrap());
static HUNK_HEADER_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@").unwrap());
...
impl FixupParser {
    ...
    pub fn detect_fixup_markers(&self, content: &str) -> Option<String> {
        // ⚡ Bolt: Using LazyLock for regex caching reduces latency during large file diff extraction and fixup detection.
    ...
    fn extract_diff_blocks(&self, content: &str) -> Result<Vec<UnifiedDiff>, FixupError> {
        // ⚡ Bolt: Cached diff_block_regex via LazyLock avoids recompilation in loops.
    ...
    fn parse_hunks(&self, lines: &[&str], block_index: usize) -> Result<Vec<DiffHunk>, FixupError> {
        // ⚡ Bolt: Cached hunk_header_regex via LazyLock avoids excessive recompilations during parsing.
