...
## 2025-03-03 - Cache Compiled Regexes in frequently called Error Redaction Functions
**Learning:** Functions that frequently instantiate and compile regular expressions (e.g., `Regex::new(r"...").unwrap()`) suffer huge performance penalties, especially when combined with string allocation (`to_string()`).
**Action:** Use `std::sync::LazyLock` to define regular expressions statically so they are only compiled once. Use `std::borrow::Cow<str>` with `regex::Regex::replace_all` to avoid unnecessary heap allocations when no pattern matches.
