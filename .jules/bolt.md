...
## 2024-06-07 - Static Regex Compilation with LazyLock
**Learning:** Dynamically compiling `regex::Regex` inside frequently called functions (such as log redaction or metadata extraction) creates a significant CPU exhaustion bottleneck. Wrapping these inside `std::sync::LazyLock` compiles them only once globally while retaining function-level scoping.
**Action:** Always wrap `Regex::new().unwrap()` in `std::sync::LazyLock` for regexes used within high-traffic execution paths to improve performance and avoid DoS vulnerabilities.
