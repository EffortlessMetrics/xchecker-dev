...
**Learning:** Dynamically compiling `regex::Regex` instances inside high-traffic functions (like metadata extraction) creates significant CPU overhead and can become a bottleneck.
**Action:** Statically cache compiled regexes using `std::sync::LazyLock::new(|| Regex::new(...).unwrap())` inside the function scope for any frequently executed code paths.
