...
## 2024-06-17 - Cache Regex compilation with LazyLock
**Learning:** The `xchecker-extraction` module repeatedly compiles regular expressions inside functions like `summarize_requirements`, `summarize_design`, and `summarize_tasks` on every invocation. Compiling regexes is computationally expensive.
**Action:** When performing regex matching, always use `std::sync::LazyLock` to cache compiled regexes as static variables to eliminate repeated regex compilation overhead.
