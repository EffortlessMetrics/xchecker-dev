...
## 2024-05-23 - Cache Regex compilation in extraction module
**Learning:** Dynamically compiling regular expressions on every function call for metadata extraction creates a significant CPU bottleneck when processing markdown artifacts.
**Action:** Prevent this overhead by statically caching compiled regexes using `std::sync::LazyLock`. Wrapping `Regex::new().unwrap()` in `LazyLock` for frequently executed code paths and declaring the static block inside the function scope prevents module-level pollution and ensures efficient, single-time compilation.
