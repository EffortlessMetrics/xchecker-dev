...
## 2024-05-23 - Static Regex caching with LazyLock in extraction
**Learning:** In high-traffic functions like metadata extraction, compiling regular expressions dynamically on every call with `Regex::new` wastes CPU cycles and acts as a bottleneck.
**Action:** Always wrap `Regex::new().unwrap()` in `std::sync::LazyLock` for frequently executed code paths to compile them exactly once on first use.
